...
# ---- Brightness/Contrast Adjustment for Low-Light ----
ENABLE_BRIGHTNESS_CONTRAST = True
BRIGHTNESS_ALPHA = 1.3  # Contrast (higher = more contrast)
BRIGHTNESS_BETA = 25    # Brightness (higher = brighter)
...
if AI_BACKEND == "gemini_live":
    from gemini_live_client import stream_chat as _backend_stream_chat
...
def apply_gamma_correction(bgr_frame, gamma=None):
    try:
        ...
        print(f"[GAMMA_CORRECTION] Error: {e}")
        return bgr_frame
    ...
    def _decode():
        try:
            ...
            if ENABLE_GAMMA_CORRECTION:
                bgr = apply_gamma_correction(bgr)
