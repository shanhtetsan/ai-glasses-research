"""Optional latest-frame HTTP client for the perception service's /v1/segment
endpoint (yolo-seg.pt: road_crossing / blind_path).
...
class YoloSegClientSettings:
    ...
    @classmethod
    def from_env(cls) -> "YoloSegClientSettings":
        enabled = _env_bool("ENABLE_YOLO_SEG", False)
        if not enabled:
            return cls(enabled=False)
        return cls(
            enabled=True,
            service_url=(
                os.getenv("YOLO_SEG_SERVICE_URL", "").strip()
...
def validate_segment_result(payload: Any, expected_frame_id: int) -> dict:
    if not isinstance(payload, dict):
        raise ValueError("segment response must be an object")
    frame_id = int(payload["frame_id"])
    if frame_id != expected_frame_id:
        raise ValueError("segment response frame_id mismatch")
...
class YoloSegShadowClient:
    ...
    async def start(self) -> bool:
        ...
        if config_error:
            ...
            print(f"[YOLO-SEG-CLIENT] disabled reason={config_error}", flush=True)
            return False
        try:
            ...
            print(f"[YOLO-SEG-CLIENT] disabled reason={self._unavailable_reason}", flush=True)
            return False
        self._running = True
        self._task = asyncio.create_task(self._run(), name="yolo-seg-http-shadow")
        self._health_task = asyncio.create_task(self._health_logger(), name="yolo-seg-http-health")
        return True
    ...
    async def _request_detection(self, frame: Any) -> None:
        ...
        try:
            response = await client.post(
                self.settings.service_url + "/v1/segment",
                content=frame.data,
            ...
            validated = validate_segment_result(response.json(), frame.sequence)
            ...
            if failure_count == 1 or failure_count % 10 == 0:
                print(
                    f"[YOLO-SEG-CLIENT] request failure count={failure_count} "
    ...
    def _maybe_log_detection_state(
        ...
        if changed and self._on_event is not None:
            try:
                self._on_event("YOLO_SEG_CHANGE", {
        ...
        print(
            f'[YOLO-SEG-DETECTION] frame_id={result["frame_id"]} '
    ...
    async def _health_logger(self) -> None:
        while self._running:
            ...
            if self._running:
                print("[YOLO-SEG-CLIENT-HEALTH] " + json.dumps(self.health(), separators=(",", ":")), flush=True)
    ...
    async def stop(self) -> None:
        ...
        if self._http_client is not None:
            try:
                ...
                print(f"[YOLO-SEG-CLIENT] HTTP client close failed reason={type(exc).__name__}", flush=True)
