...
int MLX90640_I2CRead(uint8_t _deviceAddress, unsigned int startAddress, unsigned int nWordsRead, uint16_t *data)
{
  ...
  while (bytesRemaining > 0)
  {
    ...
    {
      //Upstream returned 0 (success) here, which silently masks a no-ACK and
      //hands back an uninitialised frame buffer. Return an error instead.
    ...
    uint8_t got = Wire.requestFrom((uint8_t)_deviceAddress, (uint8_t)numberOfBytesToRead);
    if (got != numberOfBytesToRead)
    {
      //Short read — bus glitch or buffer too small. Fail loudly instead of
      //silently returning half-filled data (which produces garbage frames).
      return (-1);
    }
    if (Wire.available())
    {
