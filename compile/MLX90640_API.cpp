...
#include <math.h>
#include <Arduino.h>   // millis() / delay() — used by the bounded data-ready wait
                       // in MLX90640_GetFrameData below. Upstream never needed it
                       // because its wait loop was an unbounded busy-spin.
...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    dataReady = 0;
    // Upstream spun here forever with no timeout and no yield, hammering the I2C
    // bus as fast as it could. Our caller holds i2cMutex across this call, so on
    // a shared bus that starves the IMU task, and on FreeRTOS a tight no-yield
    // loop on core 0 (shared with the WiFi/LWIP tasks) can hold the CPU long
    // enough to look like a hang. Bounded wait + yield instead:
    //   - refresh rate 0x05 (16Hz) => a frame is ready within ~63ms; 200ms is
    //     ~3x headroom, so a timeout means something is genuinely wrong.
    //   - the 1ms delay lets the IMU task grab the bus and lets WiFi breathe.
    {
        ...
        while (dataReady == 0)
        {
            ...
            if (dataReady != 0) break;

            if (millis() - _start > 200)
            {
                return -8;   // timed out waiting for data-ready
            }
            delay(1);        // yield: never busy-spin while holding the bus
        }
    }
