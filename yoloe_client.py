"""Optional latest-frame HTTP client for the perception service's
/v1/obstacles endpoint (yoloe-11l-seg.pt: open-vocabulary obstacle whitelist,
prompted once at service load — see yolo_service/app.py's
_load_obstacle_model).

Mirrors yolo_client.py's shape (settings, DetectionCache, latest-frame
polling loop, freshness/health telemetry). No target-stability/guidance
tracking here — this detector does not participate in hand-target guidance
fusion, only in the PERCEPTION_STATE objects merge.
"""
...
class YoloeClientSettings:
    ...
    @classmethod
    def from_env(cls) -> "YoloeClientSettings":
        enabled = _env_bool("ENABLE_YOLOE_OBSTACLES", False)
        if not enabled:
            return cls(enabled=False)
        return cls(
            enabled=True,
            service_url=(
                os.getenv("YOLOE_SERVICE_URL", "").strip()
...
def validate_obstacle_result(payload: Any, expected_frame_id: int) -> dict:
    if not isinstance(payload, dict):
        raise ValueError("obstacle response must be an object")
    frame_id = int(payload["frame_id"])
    if frame_id != expected_frame_id:
        raise ValueError("obstacle response frame_id mismatch")
    ...
    for item in objects:
        ...
        validated_objects.append({
            ...
            "mask_coverage_norm": _clamp01(item["mask_coverage_norm"]),
        })
    return {
        "frame_id": frame_id,
...
class YoloeShadowClient:
    ...
    async def start(self) -> bool:
        ...
        if config_error:
            ...
            print(f"[YOLOE-CLIENT] disabled reason={config_error}", flush=True)
            return False
        try:
            ...
            print(f"[YOLOE-CLIENT] disabled reason={self._unavailable_reason}", flush=True)
            return False
        self._running = True
        self._task = asyncio.create_task(self._run(), name="yoloe-http-shadow")
        self._health_task = asyncio.create_task(self._health_logger(), name="yoloe-http-health")
        return True
    ...
    async def _request_detection(self, frame: Any) -> None:
        ...
        try:
            response = await client.post(
                self.settings.service_url + "/v1/obstacles",
                content=frame.data,
            ...
            validated = validate_obstacle_result(response.json(), frame.sequence)
            ...
            if failure_count == 1 or failure_count % 10 == 0:
                print(
                    f"[YOLOE-CLIENT] request failure count={failure_count} "
    ...
    def _maybe_log_detection_state(
        ...
        print(
            f'[YOLOE-DETECTION] frame_id={result["frame_id"]} '
    ...
    async def _health_logger(self) -> None:
        while self._running:
            ...
            if self._running:
                print("[YOLOE-CLIENT-HEALTH] " + json.dumps(self.health(), separators=(",", ":")), flush=True)
    ...
    async def stop(self) -> None:
        ...
        if self._http_client is not None:
            try:
                ...
                print(f"[YOLOE-CLIENT] HTTP client close failed reason={type(exc).__name__}", flush=True)
