...
class GeminiLiveClient:
    def __init__(self):
        ...
        # Serializes ALL writes to the Live session's websocket.
        #
        # send_audio() fires ~50x/sec from /ws_audio's loop while send_image()
        # fires from a separate task on every 15th camera frame. Both end up in
        # session.send_realtime_input() -> a bare `await self._ws.send(...)` in
        # the google-genai SDK (live.py:346). The SDK has NO lock or queue, so two
        # tasks can interleave writes onto one websocket. The Live API expects a
        # single ordered realtime-input stream per session, and rejects interleaved
        # writes with 1008 ("policy violation / operation was aborted").
        #
        # This lock makes every send atomic w.r.t. every other send.
    ...
    async def _send_realtime(self, **kwargs) -> bool:
        """Single serialized entry point for every realtime write.

        All sends go through here so writes to the shared websocket can never
        interleave (see _send_lock in __init__). Also re-checks the session
        *inside* the lock: a task created before a drop can otherwise reach its
        await after _reconnect_with_backoff set self.session = None, and
        dereference a stale/None session.
        """
        ...
        async with self._send_lock:
            ...
            if not self.connected or session is None:
                return False          # dropped/reconnected while we waited
            try:
                await session.send_realtime_input(**kwargs)
                return True
            except Exception as e:
                print(f"[Gemini Live] realtime send failed: {e}", flush=True)
                return False

    async def send_text(self, text):
        await self._send_realtime(text=text)
